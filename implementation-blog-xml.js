//a Blog That Updates on the Fly From an XML-Based RSS Feed
...
setInterval(function () {
  ...
  ajax({
    ...
    data: "xml",
    // Get the current RSS feed (with the latest posts in it)
    url: "./?feed=rss&paged=1",
    // Watch for when the RSS feed has been successfully retrieved
    onSuccess: function (rss) {
      // We're loading the new posts into the <div>
      // that has an ID of "content"
      var content = document.getElementById("content");
      // Get the URL of the most recent post (to make sure that
      // we don't do any duplicate posts)
      ...
      for (var i = 0; i < items.length; i++) {
        ...
        if (getData(items[i]).link == recentURL) break;
        // Add the new item to the temporary array
        newItems.push(items[i]);
      }
      // Go through all of the new items, in reverse order, to make
      // sure that they're in the right order when they go into the site
